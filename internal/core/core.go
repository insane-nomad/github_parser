...
type RepositoryDatas []RepositoryData
...
func GetfirstRepo(agent *fiber.Agent) RepositoryData {
	var firstRepoData RepositoryData
	agent.Add("Authorization", "Bearer "+config.GithubToken)
	req := agent.Request()
	req.Header.SetMethod(fiber.MethodGet)
	req.SetRequestURI("https://api.github.com/search/repositories?q=stm32&sort=updated&order=asc&per_page=1&page=1")
	...
	if err := json.Unmarshal(response, &firstRepoData); err != nil {
		fmt.Println(err)
	}
